...
# 2023
# 07-07 修改了 self.max_time_offset_camera_sys self.max_time_offset_camera_sys 满足系统报文误差1s以内
# 12-06 update 940 timezone change.(zj)

# 2024
# 01-19 sure system time begin at "2024-1-1 00:00:00"
# 01-20 set system time to RTC.
# 03-01 add "ntpdate -u".
...
class SyncNtpDate(object):
    ...
    def __init__(self, near_camera_IP="192.168.8.12", far_camera_IP="192.168.8.11", ntp_server_url='time.nist.gov',
        ...
        self.max_time_offset_camera_sys = 2.5
        self.max_time_offset_sys_ntpser = 1.0
        ...
        self.sys_timezone = time.tzname
        self.change_timezone = False
        self.timezone = 0
    ...
    def getNtpTime_old(self, ntp_server_url):
        ...
        try:
            ...
            if "Windows" not in platform.platform():
                time.tzset()
                self.timezone = time.timezone * (-1) # linux timezone = -1 * (940 camera timezone)
                if all(x == y for x, y in zip(self.sys_timezone, time.tzname)):
                    self.change_timezone = False
                else:
                    self.change_timezone = True
            return ntp_stats.tx_time
        except Exception as e:
            # traceback.print_exc()
            print(f"getNtpTime error={e},ntp_server_url={ntp_server_url}")
            if time.time() < 1704038400: # 1704038400 is "2024-1-1 00:00:00"
                print(f"luoccc first compare time. system time is older than 2024-1-1 00:00:00. change system time once.")
                system_to_20240101_timestamp = 1704038400
                system_to_20240101_timeStruct = time.localtime(system_to_20240101_timestamp)
                self.setNanoTime(system_to_20240101_timeStruct)
                time.sleep(0.5)
            return time.time()
    ...
    def syncTime_zy(self, ntp_server_url):
        try:
            ...
            # 如果nano时间和ntp offset_s大于self.max_time_offset_sys_ntpser则更新nano时间、远焦相机时间、近焦相机时间
            if abs(offset_s_ntp_nano) > self.max_time_offset_sys_ntpser:
        ...
        try:
            if "armv7l" not in platform.platform() and "generic" not in platform.platform():
                ...
                if abs(offset_s_ntp_farCamera) > self.max_time_offset_camera_sys:
            ...
            if abs(offset_s_ntp_nearCamera) > self.max_time_offset_camera_sys:
                self.setNearCameraTime(time.localtime())
            # 调试打印信息
            print(f"syncTime getTime_ntp={ntp_server_datetime_str},getTime_nearCamera={timeStr_nearCamera}")
            print(f"syncTime offset_s_ntp_nano={offset_s_ntp_nano},"
                  f"offset_s_ntp_nearCamera={offset_s_ntp_nearCamera}")
    ...
    def syncTime_940(self, ntp_server_url):
        try:
            ...
            cmd = f"echo {self.sudo_password} | sudo ntpdate -u {ntp_server_url}"
            ret = os.system(cmd)
            if self.using_camera_class == "940" and ret != 0:
                ...
                try:
                    ...
                    '''
                    print("***************************************")
                    # old version. get_far_DeviceTimeX() return time_str.
                    # timeStr_farCamera_pre = self.Camera_940.get_far_DeviceTimeX()
                    # timeStr_nearCamera_pre = self.Camera_940.get_near_DeviceTimeX()

                    # 2023-12-06 zj version. get_far_DeviceTimeX() return time_str, timezone.
#                    timeStr_farCamera_pre, far_camera_timezone = self.Camera_940.get_far_DeviceTimeX()
#                    timeStr_nearCamera_pre, near_camera_timezone = self.Camera_940.get_near_DeviceTimeX()
#
...
#                    if abs(offset_s_camera_nano) > self.max_time_offset_camera_sys:
...
#                    offset_s_ntp_nano = offset_s_camera_nano
                ...
                # 850 camera
                # sure system time begin at "2024-1-1 00:00:00"
                if time.time() < 1704038400: # 1704038400 is "2024-1-1 00:00:00"
                    system_to_20240101_timestamp = 1704038400
                    system_to_20240101_timeStruct = time.localtime(system_to_20240101_timestamp)
                    self.setNanoTime(system_to_20240101_timeStruct)
        ...
        try:
            ...
            print("11111111")
            timeStr_farCamera, far_camera_timezone = self.Camera_940.get_far_DeviceTimeX()

            timeFloat_farCamera = time.mktime(time.strptime(timeStr_farCamera, "%Y%m%dT%H%M%S"))
            offset_s_ntp_farCamera = timeFloat_farCamera - time.time()
            self.timezone = time.timezone * (-1)
            offset_timezone_farCamera = int(far_camera_timezone) - self.timezone
            if abs(offset_s_ntp_farCamera) > self.max_time_offset_camera_sys or abs(offset_timezone_farCamera) > 0:
                ...
                # self.Camera_940.set_far_DeviceTime(time_str=time.strftime("%Y-%m-%d/%H:%M:%S", time.localtime())) # 只修改时间
                self.Camera_940.set_far_DeviceTimeX(time_str=time.strftime("%Y-%m-%d/%H:%M:%S", time.localtime()),
            ...
            # 2023-12-06 zj version. get_far_DeviceTimeX() return time_str, timezone.
            timeStr_nearCamera, near_camera_timezone = self.Camera_940.get_near_DeviceTimeX()

            timeFloat_nearCamera = time.mktime(time.strptime(timeStr_nearCamera, "%Y%m%dT%H%M%S"))
            offset_s_ntp_nearCamera = timeFloat_nearCamera - time.time()
            self.timezone = time.timezone * (-1)
            offset_timezone_nearCamera = int(near_camera_timezone) - self.timezone
            if abs(offset_s_ntp_nearCamera) > self.max_time_offset_camera_sys or abs(offset_timezone_nearCamera) > 0:
                print(datetime.now().strftime("%Y-%m-%d/%H:%M:%S"))
                print(time.strftime("%Y-%m-%d/%H:%M:%S", time.localtime()))
                print("++++++")
                ...
                self.Camera_940.set_near_DeviceTimeX(time_str=time.strftime("%Y-%m-%d/%H:%M:%S", time.localtime()),
                                                     timezone=self.timezone)
                timeStr_nearCamera, near_camera_timezone =self.Camera_940.get_near_DeviceTimeX()
    ...
    def get_ntp_url_from_xml(self):
        # 从xml文件获取ntp url,如果失败则使用'time.nist.gov'
        ntp_server_url = '119.28.183.184'    # 默认ntp_server_url
        try:
...
if __name__ == '__main__':
    ...
    try:
        threads = threading.Thread(target=ntp.runTask)
